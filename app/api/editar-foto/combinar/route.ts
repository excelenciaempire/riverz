  ...
  try {
    ...
    const { data: generation, error: genError } = await supabase
      ...
      .insert({
        ...
        status: 'pending',
        input_data: { prompt, format: format || '1:1', imageCount: imageIndex },
        cost: creditsCost,
      })
      .select()
      .single();
    ...
    const deductResponse = await fetch(`${process.env.NEXT_PUBLIC_APP_URL}/api/credits/deduct`, {
      ...
      body: JSON.stringify({
        ...
        description: `Combinar Imágenes #${generation.id}`,
      }),
    });
    ...
    return NextResponse.json(
      { error: error.message || 'Failed to combine images' },
      { status: 500 }
    );
  }
}
